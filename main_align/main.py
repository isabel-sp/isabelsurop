...
captured_img = np.zeros((512,512,3), np.uint8)
temp_clicked = None
snspd = None
wvguide = None
    ...
    if event == cv2.EVENT_LBUTTONDOWN:
        ...
        if not click:
            ...
            if click == 'snspd':
                ...
                x_found = img_process.line_center_x(captured_img, x, y)[0]
                snspd = (x_found, y)
            elif click == 'wvguide':
...
while True:
    ...
    cv2.imshow("Captured Image", display_functions.draw_buttons(captured_img, temp_clicked, snspd, wvguide))
